...
本项目是一个面向本科毕业设计展示的多模态感知数据采集与分析平台。系统以“异构设备统一接入、传感器数据规范化处理、统一存储与可视化展示”为基础，进一步加入历史数据回溯、按时间段查询与对齐导出、空间环境状态分析和 IMU 活动识别等功能，用于演示智能空间中摄像头、环境传感器和运动传感器数据从采集、存储、回放到分析应用的完整闭环。
